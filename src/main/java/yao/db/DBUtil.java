 ...
 *
 * private static Object getValueFromRs(ResultSet rs, String fieldName, Type t)
 * throws SQLException 此接口有个小的问题就是如果，获取的字段值是空值或者为null，而你自己的需求就是想要获取的字段为一个
...
public class DBUtil {
     ...
     * @param con 数据库jdbc链接
     * @param sql 执行的sql语句
     * @return 返回查询的记录数，记录存储在集合List里面， 里面的元素是集合Map,key是数据库中的字段类型，value是 字段类型对应的值
     * @throws SQLException
     */
    ...
    /**
     * *
     * 这个函数与initObjectFromRsIfExist函数实现的功能是一样，只是 没有判断dao中的字段是否与数据库记录所定义的字段是一样的,
     ...
     * *
     *
     * 把数据库的一条记录映射到相应的dao对象中， 如果dao中的字段与数据库字段不一致，返回的就是dao数据类型定义的默认值 如：dao的字段long
     * vehicleID;而数据库的字段是vehicle_id，那么返回的 就定义的默认值0.
     *
     * @param rs 查询的结果集
     * @param c 结果集一条记录，而一条记录所对应的dao类
     * @return
