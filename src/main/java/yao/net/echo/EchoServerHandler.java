...
public class EchoServerHandler extends SimpleChannelInboundHandler<DatagramPacket>{
    ...
    protected void channelRead0(ChannelHandlerContext ctx, DatagramPacket packet)
        ...
        if(str.equalsIgnoreCase("L")){
            //保存到addr1中 并发送addr2
            addr1 = packet.sender();
            System.out.println("L 命令， 保存到addr1中 "+addr1.getAddress().toString().replace("/", "")
                    +" "+addr1.getPort());
            System.out.println("L 命令， 保存到addr1中 "+addr1.getHostName()+addr1.getPort());
        }else if(str.equalsIgnoreCase("R")){
            //保存到addr2中 并发送addr1
            addr2 = packet.sender();
            System.out.println(addr2.getAddress().toString().replace("/", "")+" "+addr2.getPort());
            System.out.println("R 命令， 保存到addr2中 "+addr2.getAddress().toString().replace("/", "")
                    +" "+addr2.getPort());
            System.out.println("R 命令， 保存到addr2中 "+addr2.getHostString()+addr2.getPort());
